...
- **/designer** — the style conversation building momentum never starts: adopt-or-craft first, taste forks settled by *looking* (named references, style tiles) — because taste is your world, not the model's default aesthetic — and one committed design language recorded in `DESIGN.md`, so every later screen speaks it. A command you type; one language per product
