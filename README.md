...
Nothing to build, nothing to run. The skills are plain markdown folders under `skills/`; what rides inside them is Claude Code's — three subagents, two judges registered when you type `/enable` or `/tenacity` and never by installation, blocking the two failures the flow has measured (a coordinator that announces a dispatch and ends its turn, a builder that hands back a claim instead of evidence) — where Claude Code's function hooks are on (`CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1`, early access) the same two judges run in-process as a Claude Mod, armed by those same two commands, with a status line naming the work in flight and its next command, the map as a pane beside the transcript (`/genius-map`), the instrument's count after every snapshot write, the work-file note carried into every compaction, and a `confirm` tool that turns a checkpoint's yes into a button only the person can press — and the format's own instrument, whose counts are injected before `/genius`, `/compact`, `/reconcile` and `/distill` read. `evals/` holds the suite `claude plugin eval` runs against the skills' own claims.
