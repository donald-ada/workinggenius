...
- **/designer** — the style conversation building momentum never starts: adopt-or-craft first, references hunted fresh for *your* product's domain (never the canon every model recalls, never one curator's template pool), taste forks settled by *looking* (style tiles, not adjectives), and one committed design language recorded in `DESIGN.md`, so every later screen speaks it. A command you type; one language per product
