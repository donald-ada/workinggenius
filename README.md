...
- **/designer** — the style conversation building momentum never starts: name the base (an existing surface, a template you like — the awesome-design-skills catalog plugs in as an on-demand base source — or nothing) and the delta you want; deltas settled by *looking* (throwaway style tiles, references hunted only when needed and priced), one committed language in `DESIGN.md` that every later screen speaks. A command you type; one language per product
...
- [bergside/awesome-design-skills](https://github.com/bergside/awesome-design-skills) (MIT) — the 67-template catalog `/designer` pulls a named base from, on demand: one template per need, never the pool
