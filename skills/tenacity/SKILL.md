...
Review the full diff of the work — from the `base:` commit recorded in the work file's frontmatter to the current tree (fall back to asking the user for the baseline if `base:` is missing). For anything sizable, spawn **one fresh reviewer subagent**, isolated from this session's history so it judges the work, not your reasoning about the work. A mid-tier model reads a diff as well as a frontier one — reserve the expensive model for building, not reviewing (measured on an ordinary feature diff; where a missed defect is expensive — money paths, migrations, security surface — escalating the reviewer to the main model is an unmeasured judgment call, recorded if taken). Hand it the diff and the work file *as files*, never pasted summaries, and scope it to them: the reviewer judges the diff against the brief — it does not re-drive the app or wander the repo (an unscoped reviewer was the most expensive single step in measured runs, at no gain over a scoped one). Scoped means purpose-bound, not blindfolded: pulling one named file to check one named finding — the caller of a changed function, the test that should have moved — is the review working; open-ended re-exploration is the cost sink. Spawn it so its result reaches you — synchronously (foreground) unless you are certain the harness re-wakes you when it finishes; ending your turn to "await the reviewer" in a context that won't be woken is a stall, not a wait. Ask for both verdicts — the review's floor, not its boundary:
...
- **And anything else worth blocking on** — a finding that deviates from no spec line and breaks no convention (a plain bug, a hole an attacker would use) is still a finding; the axes structure the review, they don't cap it.
...
- Slices carry issue numbers? Sweep them: an issue still open on work about to be marked done is drift — close it, pointing at the evidence.
