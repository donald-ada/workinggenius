...
- **Read before proposing.** The snapshot, the `CONTRACT.md` beside it, and the log's branches and keys as `measure.py anchors <slug>` lists them. Destinations are decided against the Slices list, so a full log read is never the price of this pass.
...
- **Into the log, in the shape the format names** — verbatim, keyed off the anchor its line already links, in that anchor's branch.
...
- **`stage: done` changes three things.** The resting shape stays whole — Problem, Decision, changelog, roster — whatever a literal reading of the question says. `CONTRACT.md` is not drained. And a log whose first line begins `distilled` — at close-out or retroactively — will not be distilled again, so anything appended after that line stays forever: say in the pass entry that content follows the distillation.

## Finishing

- **Mark the pass** — `## compacted-<date>` in the log's `log/compacted.md` branch, naming what moved and where, with a link to it left in the snapshot beside the drained-from section, so the pass is reachable like everything else the log holds.
