...
- **Mark the pass** — `## compacted-<date>` in the log, naming what moved and where, with a link to it left in the snapshot beside the drained-from section, so the pass is reachable like everything else the log holds.
