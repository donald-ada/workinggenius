...
- A section is corrected in place; a record is corrected by appending to it (`errata` skill). The section is what the next session acts on, so a wrong line there is an instruction and gets rewritten. A record is what was written then, so its wrong line stays and a dated erratum below it names what overturned it. Neither is ever silently removed — a line deleted without its retirement note leaves a hole that reads like a stage that never ran.
