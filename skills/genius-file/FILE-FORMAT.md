...
Retire, like the snapshot's Open drain, is the case where the line itself is what leaves, so no link can survive in its own place. It moves up one level exactly as draining does: **`BACKLOG.md` carries `[retired](BACKLOG.log.md#<key>-<date>)` at its foot, one per retirement, never one accumulating them** — the log is append-only, so each retirement is its own dated entry and no later one can be filed under an earlier anchor. A retirement whose link got overwritten is text nobody can reach, which is the hole `errata` warns reads exactly like a stage that never ran.

**Starting the work is the fourth way a line leaves, and the one that writes no log entry**: the line goes because the new work file is its home now (the `genius-file` skill carries that rule), and that file — listed by `ls .genius/`, opened by every status view — holds what the seed was pointing at. Nothing is lost, so nothing needs displacing. ⚠ **The line between this and Retire is which came first.** A seed that *became* the work leaves this way. A seed an already-running work absorbed on its way past is retired, with the absorbing work named as what retired it — because there the work file was never that seed's home, and without the log entry nothing records that the seed was answered rather than forgotten.
