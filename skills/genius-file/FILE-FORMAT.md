...
⚠ **The cross-work files are not a piece of work, whatever their shape.** `BACKLOG.md` with `BACKLOG.log.md` beside it matches the flat two-file work below exactly, and `/migrate` reads shape rather than names — so the exception is stated here: `.genius/`'s own files are never migrated into a folder. They span works; a folder would say they belong to one.
