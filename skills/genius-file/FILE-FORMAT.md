...
`.genius/` itself keeps only what spans works: `BACKLOG.md` with `BACKLOG.log.md` behind it ([BACKLOG-FORMAT.md](BACKLOG-FORMAT.md)), `DECIDED.md`, `HISTORY.md`. So `ls .genius/` lists pieces of work, one entry each, however many artifacts a work carries.

**Three files, three growth laws — that is the design of a work's folder.** (`.genius/`'s own cross-work files add one more — [BACKLOG-FORMAT.md](BACKLOG-FORMAT.md).) The snapshot's size follows *scope*: requirements can change ten times and it is still short enough that a cold session reads it whole before acting. The log's size follows *time*, which costs nothing, because nobody reads a log whole. The contract's size follows *slice count* — it is the third law, and it is the one that breaks a two-file design: a convention the build introduced binds the slices that haven't been built yet, so it can be neither deleted nor left in a file whose size must follow scope.
...
has a growth law of its own (so do Open, with the snapshot below, and `BACKLOG.md`, in its own file — the laws are named for what they govern rather than numbered, since any reordering silently breaks an ordinal): it grows by slice count like the contract, but unlike the contract it cannot move
...
`BACKLOG.md` with `BACKLOG.log.md` behind it has a growth law and a two-file shape of its own — it grows by how many discoveries the flow made, and nothing leaves it except into its log or into the work it became. Its format lives in [BACKLOG-FORMAT.md](BACKLOG-FORMAT.md): the four ways a line leaves (retire, merge, correct, start the work), what each writes to the log, the order rule, and why the cross-work files are never a piece of work whatever their shape. Split out because a slice close never needs it and pays for every character loaded; the passes that do — `/genius`, `/triage`, `/reconcile`, `/migrate`, `/distill` — open it there.
