...
**The seed's shape:** `- **<what>** — <why it matters>. From [<slug>](<slug>/<slug>.log.md#<anchor>), <date>.` One physical line, 300 characters at most (`record-prose` holds why); the `From` link is relative to `.genius/` (FILE-FORMAT's link rule). A seed found outside any work (a `diagnose` hunt, a stray edge) has no log to anchor to, so its `From` names the commit that surfaced it instead: `From <short-hash>, <date>.`
