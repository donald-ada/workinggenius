...
**The seed's shape:** `- **<what>** — <why it matters>. From [<slug>](<slug>/log/<branch>.md#<anchor>), <date>.` One physical line, 300 characters at most (`record-prose` holds why); the `From` link is relative to `.genius/` (FILE-FORMAT's link rule). A seed found outside any work (a `diagnose` hunt, a stray edge) has no log to anchor to, so its `From` names the commit that surfaced it instead: `From <short-hash>, <date>.`

`BACKLOG.md` grows by how many discoveries the flow made, and a per-line bound is no bound on the file ([FORMAT-EDGES.md](FORMAT-EDGES.md)). So it gets the same two-file shape a work gets, and the same invariant: **nothing leaves `BACKLOG.md` except into `BACKLOG.log.md` or into the work file it became, and a link is left behind wherever there is still a line to carry it.** `BACKLOG.log.md` sits beside it, is append-only, and is created at its first entry, not before. An entry's key is a short descriptive slug plus the date, `<slug>-<date>` — `stale-vpp-tooltip-2026-08-29` is one whole key — because a seed can be corrected, merged and retired across months and those are the same seed, so the key cannot be unique by construction the way a numbered slice is. The seed points at one with `[detail](BACKLOG.log.md#<slug>-<date>)`.

Three moves route a seed's text into the log instead of deleting it:
...
**`BACKLOG.log.md` is not a third exit from the snapshot.** An Open item that is work in its own right routes through *its own work's* log, with the seed pointing there, because nothing leaves a snapshot except into its own log or `CONTRACT.md`, and a seed is lossy by design ([FILE-FORMAT.md](FILE-FORMAT.md), the invariant and the Open drain). This log takes only what leaves `BACKLOG.md` itself.
