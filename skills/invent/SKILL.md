...
5. Record the paths in the snapshot as state waiting for the attack — shape, what it makes easy, what it costs, what it was checked against — and the whole of what the inventors returned under `## invention` in the log. Run inside `/discern`, go straight on to its attack in the same command. Typed alone, write `next: /discern <slug>`, the exact command, because with two works at the same stage a bare stage name does not say which one moves.
