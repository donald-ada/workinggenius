...
- **Scope: done, has a log, undistilled.** Only works at `stage: done` with a `<slug>.log.md` whose first line is not `distilled`. In-flight logs are never touched, whatever the argument says, because the repo can only answer for work that is finished and verified, and this deletion is `errata`'s one carve-out for exactly that work; a slug that names in-flight work is skipped for that reason. The scan is the instrument's — where a policy notice shows instead, run it before proposing:
...
- **Mark the pass.** A new first line, `distilled retroactively, <date>`, goes above the log's first entry heading, never in place of it, because that heading is an anchor something may still link. Links that pointed at removed entries leave with them — a slice line keeps its check and its words, never a dead anchor.
