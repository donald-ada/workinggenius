...
- **A pointed-at entry stays, whatever its category.** Before an entry leaves, check nothing outside the work's own folder links to it — `.genius/DECIDED.md` above all, and `.genius/BACKLOG.md`, whose every line carries a `From [<slug>](…#<anchor>)` back into some log. An anchor the index points at is load-bearing; deleting it breaks the door the index exists to be.
