...
- **Speak the project's language** — terms from `CONTEXT.md` where the project keeps one, never shorthand this conversation invented. Leaning on a concept no shared vocabulary covers? Define it now, and if it is a project concept rather than a general one, record it through the `domain-glossary` skill, which owns what the glossary admits and its shape. An explanation that coins a term and doesn't write it down has to coin it again next time.
...
A second `/waitwhat` on the same topic is a finding: the gap isn't the wording, it's a missing shared concept. Name it, record it through the `domain-glossary` skill when it is the project's own, then explain with it — the repair feeds the project's memory, so the next first draft lands on its own.
