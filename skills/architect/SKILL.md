...
- **Build-or-adopt is the first fork, and it belongs to the user.** Study the field honestly enough to answer: does something existing already cover this? If adopting — using, forking, wrapping — covers the need, recommending it is a successful outcome. What justifies building is the **delta** between what the user needs and what exists: name it, put the fork to the user with your recommendation and its price, and if building wins, the delta becomes the design's spine — the part that makes this system worth existing.
...
Record the study and the decisions where the project keeps its records, so the next session reads the record instead of re-deriving it. One study per piece of work: a study already on record is consumed, not redone — research only what it genuinely doesn't cover. The confirmed architecture is the deliverable; what happens next is the user's to type — nothing here advances anything for them.
