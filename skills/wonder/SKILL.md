...
- **Price the questions that fork the cost; force the trade-offs.** The interview is a trade, not an extraction: you hold cost information the user doesn't. When an answer forks the scope, say the price *in the question* ("supporting resume across machines roughly doubles the state design — is that scenario real here?") — an unpriced option list makes the user choose blind. Prices come from the repo and design reality you just read, never invented percentages. And when desirables compete, don't ask importance — force the choice: "if only one lands first, which?" A ranking under constraint is worth ten adjectives.
...
Territories the gate needs covered — a floor the interview must have reached by its end, not a script to walk in order. The conversation sets the path and the questions; these only say where it must have been (skip any already settled):
