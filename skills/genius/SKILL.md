...
Announce the cost with the call. The full flow is insurance priced in tokens — a measured six-stage run cost 11× its no-plugin baseline (`evals/RESULTS.md`) — so the announcement names what the stages are buying ("two capable designs exist and the wrong one is expensive to unwind") or what makes them safe to skip ("one seam, no new concept"). Write the call and its reason as the `**Sizing:**` line under the work file's title (the `genius-file` skill owns the format): the close-out judges sizing calls against outcomes, and an unrecorded call can't be judged. "Just do it" on full-flow-shaped work buys delegated mode, not express — trimming checkpoints is a mode choice; skipping judgment on work that needs it is how plausible-but-wrong ships.

For the full flow, also settle the mode — recommend one with a reason and let the user pick: `mode: guided` (checkpoints as written), `delegated` (run on your recommendations, one stop at the Galvanizing breakdown), or `auto` (no stops; the user explicitly asked for hands-off). Modes live in the work file; the `genius-file` skill owns their semantics.
