...
| | Invention — generate options | runs inside `/discern` | anchoring on the first idea |
...
State lives in `.genius/<slug>/<slug>.md` — the bounded snapshot a cold session reads whole — with its history in a log shaped as a tree (`<slug>.log.md`, one branch per stage or slice under `log/`) and what binds the unbuilt slices in `CONTRACT.md`, beside it (the `genius-file` skill owns the discipline). Each stage ends when its one threshold is honestly true; each is a command the user types — Invention excepted, which runs inside `/discern` because nothing in it waits on the user — and one they don't type simply doesn't run — its absent section is the record. Four layers run underneath: the `domain-glossary` skill keeps the shared language in `CONTEXT.md`, the `decision-record` skill keeps the index of settled decisions in `.genius/DECIDED.md`, the `blindspot` skill hunts the unknowns the stages can't reach, and the `errata` skill corrects what any of them got wrong.

## What to do when invoked

The counts first, taken as this command was invoked: the block below is replaced by the instrument's output before you read it, and where a policy notice shows instead (shell injection disabled), run it before saying a number, because a number said without one is a guess:
...
**An idea** → start it: create the work file (`genius-file` skill) and open the Wonder interview. An idea that arrives as a tracker issue is read first, and the work file records which issue it answers; one taken from `.genius/BACKLOG.md` leaves it the way [BACKLOG-FORMAT.md](../genius-file/BACKLOG-FORMAT.md) says a started seed leaves, because a line removed before the work holds its content is a discovery lost with nothing left to record it. Dropping a stage is the user's call, made by not typing it — never a package you propose. And stages trade; depth doesn't: a stage worth running is worth running at full depth, because a skimmed stage pays the ceremony and buys nothing. Reaching the next command is not progress — meeting this one's threshold is.
