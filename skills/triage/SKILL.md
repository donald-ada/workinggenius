...
With an argument, only the line whose subject it names, and only the two questions one line can answer — should it be here, and has it outgrown a line. Pairing needs its candidates and order needs the file, so say plainly that those were not asked rather than answering them from one line.
...
Done when every line has been through all four questions and carries a ruling or an explicit no-change, and everything undecidable inside `.genius/` is named — a report of what was looked at cannot be told apart from one of what was left alone.
