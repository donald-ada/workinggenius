...
Every file under `.genius/` is read by two audiences the writer never meets: a cold session with no memory of the conversation, and the owner of the project, months later, looking for one fact. A record written for the conversation that produced it reads as complete to its writer and fails both of them: the cold session acts on a fact broader than what was checked, and the owner cannot find the one fact among the narration. The `genius-file` skill owns *where* things go; this owns *how the sentences carrying them are made*.

The concept: **write for someone who wasn't there, finding the one fact they came for, believing it exactly as far as it deserves.**
...
- **Route before trimming — a seed's detail is displaced, never deleted.** An entry over its pot usually carries facts recorded nowhere else, so what the trim takes goes where [BACKLOG-FORMAT.md](../genius-file/BACKLOG-FORMAT.md) routes displaced seed text, because a trim that lands nowhere is a deletion with a shorter line to show for it. An entry already inside its pot is left exactly as it is, because nothing it holds needs a new home. A link the original lacked is an inference: verify that the anchor records the thing before writing the link, because a pointer to an entry that lacks the detail loses it as surely as a deletion does.
