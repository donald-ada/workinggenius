...
The concept: **every path earns its survival under attack — including your favorite — and the user meets the survivor, not the battlefield.** Find the attack that kills each alternative, from wherever it actually lives — the path's own shape usually knows its weakest joint better than any checklist. Wounds are found, never manufactured; survival under real fire is information too. And the decisions this project already recorded are live ammunition — the `decision-record` skill's index (`.genius/DECIDED.md`) is the door to every settled fight, and finished work files hold the rest: a path that contradicts one either dies of it, or survives by naming the decision it overturns and why. A record nobody reads is decoration; this is where it earns its keep.
