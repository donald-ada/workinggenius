...
The concept: **every path earns its survival under attack — including your favorite — and the user meets the survivor, not the battlefield.** Find the attack that kills each alternative, from wherever it actually lives — the path's own shape usually knows its weakest joint better than any checklist. Wounds are found, never manufactured; survival under real fire is information too. And the decisions this project already recorded are live ammunition — wherever it keeps them (`docs/adr/` — the `decision-record` skill's home — and the kill-reasons in finished work files): a path that contradicts one either dies of it, or survives by naming the decision it overturns and why. A record nobody reads is decoration; this is where it earns its keep.
...
Record why the winner won and one line per rejected path on what killed it — an unrecorded kill-reason gets re-proposed in six weeks. A decision that would leave a future reader asking "why on earth?" may deserve a decision record — the `decision-record` skill holds the bar; most decisions miss it and stay in the work file. If every path dies, that's a finding, not a failure: back to `/invent` with the wounds as constraints, or back to `/wonder` if the wounds indict the problem itself.
