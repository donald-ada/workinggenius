...
4. Present the commitment as consequences, and each kill as the attack that broke it; the confirmation goes through the `confirm` tool where the session offers it (`genius-file` skill). A menu only where the fork is the user's world; teach first where they cannot evaluate it, or record an honest `assumed:`.
