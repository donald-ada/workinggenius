...
Then red → green, at the agreed seams, until the criteria are covered: the failing test first, watched failing; the minimal code that passes; typecheck (and lint, where fast) after each green. Write each criterion's evidence while the output is on screen — appended to the `slice-N-wip` entry as it runs, one line per criterion, the command and its result (`record-prose` skill) — and carry those lines whole into the `slice-N` entry the close writes, because evidence retyped from memory at the close is a claim, not a record.
