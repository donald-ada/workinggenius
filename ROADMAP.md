...
3. *All cost guidance is vibes.* Competing tiering (wshobson's five tiers, gstack
   routing) ships without measurement. Our `RESULTS.md` runs — including one
   recorded reversal of our own rule — are the only instrumented cost model found.
...
- **1.1 Run the unrun scenarios.** Every scenario marked *(not yet run)* gets its
  three-run majority with a baseline arm, graded and recorded.
  *Accept:* no *(not yet run)* markers remain in `evals/scenarios/`; each cleared
  marker has a `RESULTS.md` row.
...
- **1.3 CI for the deterministic layer.** `gates.test.sh` runs on every push.
  *Accept:* the workflow file exists; one deliberately-broken branch run shows
  ...
  main's state.
- **1.4 Re-measure the full flow post-sizing.** The 11× number predates priced
  sizing. Re-run the same class of task through `/genius` now that express is the
  announced default for small work.
  *Accept:* a new RESULTS entry with the same metering discipline; README cites
  the newer number (or honestly reports it didn't move).
...
Nobody ships in-loop budgeting; we have the measurements to. Sizing priced the
*entry*; this phase prices the *journey*.
...
- **2.3 Express-share telemetry.** `/genius` status reports the express:full
  ...
  *Accept:* status output shows the ratio when ≥3 done files exist.
...
- **3.3 Ship it separately.** Own repo/marketplace entry, its own README in the
  ...
  *Accept:* installable standalone; one project that is not workinggenius uses
  it in anger (issue tracker or testimonial as evidence).
...
- **4.3 Invite falsification.** README asks readers to run the baseline arm
  ...
  *Accept:* template exists; first external reproduction (or refutation —
  equally a win) recorded in RESULTS.
...
3–5 are independent of each other and can interleave; each is express-sized or
gets its own work file under this repo's own flow. This roadmap is a Galvanizing
artifact: it gets the same treatment as any plan — attacked before trusted,
revised when reality wins, its deviations recorded rather than improvised.
