...
Setup shorthand: **[tracked]** = a checkout-discounts work file authored at `.genius/checkout-discounts.md` in the current FILE-FORMAT (two options recorded), its stage records in `.genius/checkout-discounts/` linked from the sections they back, with `stage:` set as the row needs. **[bare]** = no `.genius/` directory.
